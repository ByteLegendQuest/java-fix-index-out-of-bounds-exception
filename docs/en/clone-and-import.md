...
`git clone https://github.com/ByteLegendQuest/java-fix-index-out-of-bounds-exception`

A project directory named `java-fix-index-out-of-bounds-exception` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-index-out-of-bounds-exception` directory which is created just now.
